 ...
 */
public class SelectPrecautionById extends MappingSqlQuery<Precaution>{
    private static final String SQL_SELECT_PRECAUTION = "SELECT pt.*,pct.name FROM precaution_table pt " +
            "INNER JOIN precaution_catalog_table pct ON(pct.id = pt.precaution_catalog_link) WHERE pt.id = :id";

    public SelectPrecautionById(DataSource ds) {
