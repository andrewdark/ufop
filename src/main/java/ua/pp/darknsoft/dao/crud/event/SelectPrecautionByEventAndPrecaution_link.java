 ...
 */
public class SelectPrecautionByEventAndPrecaution_link extends MappingSqlQuery<Precaution>{
    private static final String SQL_SELECT_PRECAUTION = "SELECT pt.*,pct.name FROM precaution_table pt " +
            "INNER JOIN precaution_catalog_table pct ON(pct.id = pt.precaution_catalog_link) " +
            "WHERE pt.check_event_link = :check_event_link AND pt.precaution_catalog_link = :precaution_catalog_link";

    public SelectPrecautionByEventAndPrecaution_link(DataSource ds) {
        super(ds, SQL_SELECT_PRECAUTION );
        super.declareParameter(new SqlParameter("check_event_link", Types.BIGINT));
        super.declareParameter(new SqlParameter("precaution_catalog_link", Types.BIGINT));
    }
