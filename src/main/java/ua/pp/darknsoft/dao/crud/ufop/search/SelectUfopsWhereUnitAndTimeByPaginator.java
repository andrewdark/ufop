 ...
 */
public class SelectUfopsWhereUnitAndTimeByPaginator extends MappingSqlQuery<Ufop> {
    private static final String SQL_SelectUfopsByPaginator = "SELECT ufop.*,u.username,ch.ufop_link FROM ufop_table ufop " +
            "INNER JOIN user_table u ON(u.id = ufop.creator_link) RIGHT JOIN check_event_table ch ON(ufop.id=ch.ufop_link) " +
            "WHERE ch.structure_catalog_link = :unit AND ch.event_date_end > (now() - :utime::interval) ORDER BY ufop.id " +
            "LIMIT :total OFFSET :pageid";

    public SelectUfopsWhereUnitAndTimeByPaginator(DataSource ds) {
        ...
        super.declareParameter(new SqlParameter("unit", Types.INTEGER));
