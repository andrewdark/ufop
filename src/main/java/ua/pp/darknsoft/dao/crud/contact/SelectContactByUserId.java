 ...
 */
public class SelectContactByUserId extends MappingSqlQuery<Contact> {
    private final static String SELECT_CONTACT_SQL = "SELECT c.*,ut.ufop_name sorganization FROM contact_table c LEFT JOIN ufop_table ut ON (ut.id = c.organization) " +
            "WHERE c.id=(SELECT contact_link FROM user_table WHERE id=:id)";

    public SelectContactByUserId(DataSource ds) {
