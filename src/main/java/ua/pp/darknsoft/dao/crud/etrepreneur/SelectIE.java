 ...
 */
public class SelectIE extends MappingSqlQuery<IndividualEntrepreneur> {
    private final static String SELECT_CONTACT_SQL="SELECT c.*,u.username FROM individual_entrepreneur_table c INNER JOIN user_table u ON(c.owner=u.id)ORDER BY c.id LIMIT :total OFFSET :pageid";
    private SelectContactById selectContactById;
    public SelectIE(DataSource ds) {
