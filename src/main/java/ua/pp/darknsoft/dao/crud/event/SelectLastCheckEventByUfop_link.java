 ...
 */
public class SelectLastCheckEventByUfop_link extends MappingSqlQuery<CheckEvent> {
    private static final String SQL_SELECT_CHECK_EVENT = "SELECT ce.*,u.username,sc.name structure_name FROM check_event_table ce " +
            "INNER JOIN user_table u ON(u.id = ce.creator_link) " +
            "LEFT JOIN structure_catalog_table sc ON (ce.structure_catalog_link = sc.id)" +
            "WHERE ce.ufop_link = :ufop_link ORDER BY id DESC LIMIT 1";

    public SelectLastCheckEventByUfop_link(DataSource ds) {
    ...
    protected CheckEvent mapRow(ResultSet resultSet, int i) throws SQLException {
        ...
        checkEvent.setStructure_catalog_name(resultSet.getString("structure_name"));
